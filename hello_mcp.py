...
async def main() -> None:
    ...
    #
    # Two nested `async with` blocks. The outer one owns the subprocess
    # (exiting it kills the process). The inner one owns the session/protocol
    # state (exiting it shuts the protocol down cleanly). The order matters
    # — you can't have a live session over a dead transport.
    async with stdio_client(server_params) as (read_stream, write_stream):
        async with ClientSession(read_stream, write_stream) as session:
            ...
            if target_tool is None:
                print("⚠ Neither `read_text_file` nor `read_file` is available "
